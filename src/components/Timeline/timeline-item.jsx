...
const TimelineItem = observer(
  ({
    ...
    const handleDirectClick = e => {
      ...
      // Crop/cut only when cut mode is active and for supported item types
      if (
        !isCutMode ||
        (item.type !== 'imageUrl' &&
          item.type !== 'image' &&
      ...
      // Compute splitPoint from the small yellow hover line (mouse X inside the item),
      // independent of the main timeline playhead.
      ...
      // Guard against splitting exactly at the edges
      if (splitPoint <= item.timeFrame.start || splitPoint >= item.timeFrame.end) {
        onSelectItemClick();
        return;
      }
